...
from OCW.load_dataset_synth import load_ICU_dataset, load_airquality_dataset, load_WiFi_dataset, get_model_size, load_synth_dataset
...
elif args.dataset in ['1K', '1M', '1K_normal', '1M_normal']:
    X = load_synth_dataset(window=args.window, dataset_name=args.dataset, time_step=5)
...
if args.prefix == 'testNumStream':
    res_df.to_csv(f'./exp_res/synth_{args.method}_{args.prefix}_{args.dataset}_window_{args.window}_eval_{args.eval_ratio}_stream_{args.stream}.csv', header=True, index=False)
else:
    res_df.to_csv(f'./exp_res/synth_{args.method}_{args.prefix}_{args.dataset}_window_{args.window}_eval_{args.eval_ratio}.csv', header=True, index=False)
